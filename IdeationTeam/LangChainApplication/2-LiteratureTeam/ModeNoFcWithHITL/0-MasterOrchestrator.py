"""
Master Orchestrator for Literature Team Pipeline (No Firecrawl, WITH Human-in-the-Loop)
This script runs all three stages sequentially in a fully automated pipeline with human-in-the-loop checkpoints.
...
def main():
    ...
    print("Mode: No FireCrawl, WITH Human-in-the-Loop")
    print("="*70)
    ...
    try:
        ...
        # ========== HITL CHECKPOINT 1 ==========
        print("\n" + "="*70)
        print("🔍 HUMAN-IN-THE-LOOP CHECKPOINT 1: Literature Review")
        print("="*70)
        print(f"\nPlease review the literature gathered in:")
        print(f"  - {batch_file}")
        print(f"  - {csv_file}")
        print(f"\nTop 5 Literature Items:")
        for i, item in enumerate(literature_batch.literature_items[:5], 1):
            print(f"  {i}. {item.title[:70]}...")
            print(f"     Source: {item.source} | Year: {item.year or 'N/A'}")
        
        proceed = input("\n✅ Approve and proceed to Stage 2? (yes/no): ").strip().lower()
        if proceed not in ['yes', 'y']:
            print("\n❌ Pipeline stopped by user. Please review and modify the literature batch before continuing.")
            return
        print("✅ Approved. Proceeding to Stage 2...")
    ...
    try:
        ...
        # ========== HITL CHECKPOINT 2 ==========
        print("\n" + "="*70)
        print("🔍 HUMAN-IN-THE-LOOP CHECKPOINT 2: Gap Analysis Review")
        print("="*70)
        print(f"\nPlease review the gap analysis in:")
        print(f"  - {gap_file}")
        print(f"  - {gaps_csv}")
        print(f"  - {graph_file}")
        print(f"\nTop 5 Research Gaps Identified:")
        ...
        proceed = input("\n✅ Approve and proceed to Stage 3? (yes/no): ").strip().lower()
        if proceed not in ['yes', 'y']:
            print("\n❌ Pipeline stopped by user. Please review and modify the gap analysis before continuing.")
            return
        print("✅ Approved. Proceeding to Stage 3...")
    ...
    try:
        ...
        # ========== HITL CHECKPOINT 3 ==========
        print("\n" + "="*70)
        print("🔍 HUMAN-IN-THE-LOOP CHECKPOINT 3: Final Synthesis Review")
        print("="*70)
        print(f"\nPlease review the synthesis outputs in:")
        print(f"  - {synthesis_file}")
        print(f"  - {review_file}")
        print(f"  - {plan_file}")
        print(f"  - {bib_file}")
        print(f"\nLiterature Review Preview:")
        print(f"  Title: {synthesis_result.literature_review.title}")
        print(f"  Sections: {len(synthesis_result.literature_review.sections)}")
        print(f"\nResearch Plan Preview:")
        print(f"  Title: {synthesis_result.research_plan.plan_title}")
        print(f"  Objectives: {len(synthesis_result.research_plan.research_objectives)}")
        if synthesis_result.research_plan.research_objectives:
            print(f"\n  Top 3 Research Objectives:")
            for i, obj in enumerate(synthesis_result.research_plan.research_objectives[:3], 1):
                print(f"    {i}. {obj.objective_title} (Priority: {obj.priority})")
        
        proceed = input("\n✅ Approve final outputs? (yes/no): ").strip().lower()
        if proceed not in ['yes', 'y']:
            print("\n❌ Final outputs not approved. Please review and modify before finalizing.")
            print("You can manually edit the output files and re-run if needed.")
            return
        print("✅ Final outputs approved!")
